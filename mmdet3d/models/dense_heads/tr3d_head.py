...
class TR3DHead(BaseModule):
    def __init__(self,
        ...
        self.SMALL_OBJECTS = [3, 5, 17, 25, 29, 31, 32, 33, 34, 35]
        self.LARGE_OBJECTS = [4, 6, 7, 10, 12, 14, 15, 24, 36]
        self.UNSEEN_OBJECTS = [4, 10, 17, 29]
    ...
    def _loss_single(self,
                     bbox_preds,
                     cls_preds,   # point wise class predictions
                     points,
                     gt_bboxes,
                     gt_labels,   
                     img_meta):
        
        # # Map labels to small or large objects
        # gt_labels = torch.where(torch.isin(gt_labels, torch.tensor(self.LARGE_OBJECTS).to(gt_labels.device)), 1, 0)
        # print(f"After {gt_labels}")
        # Assign gt label to each point
      
        assigned_ids = self.assigner.assign(points, gt_bboxes, gt_labels, img_meta)  # For each point get index in gt label if valid else -1
        ...
        pos_mask = assigned_ids >= 0  # For each point get 1 if valid gt label is assigned 0 if not
        
        # For each point get assign gt_label if valid, if background assign some arbirary number (1000)
        if len(gt_labels) > 0:
            # cls_targets = torch.where(pos_mask, gt_labels[assigned_ids], n_classes)
            cls_targets = torch.where(pos_mask, gt_labels[assigned_ids], 1000)
        else:
            # cls_targets = gt_labels.new_full((len(pos_mask),), n_classes)
            cls_targets = gt_labels.new_full((len(pos_mask),), 1000)
              
        #Find unseen mask by using the generated cls_targets
        unseen_mask = torch.isin(cls_targets, torch.tensor(self.UNSEEN_OBJECTS).to(cls_targets.device))
        # Update assigned_ids to -1 for unseen classes
        # assigned_ids = torch.where(unseen_mask, -1, assigned_ids[unseen_mask])
        assigned_ids[unseen_mask] = -1 # All unseen object ids should change to -1
        
        # Update pos mask and cls_targets
        ...
        cls_targets = torch.where(torch.isin(cls_targets, torch.tensor(self.LARGE_OBJECTS).to(cls_targets.device)), 1, cls_targets)
        cls_targets = torch.where(torch.isin(cls_targets, torch.tensor(self.SMALL_OBJECTS).to(cls_targets.device)), 0, cls_targets)
        cls_targets = torch.where(torch.isin(cls_targets, torch.tensor([1000]).to(cls_targets.device)), n_classes, cls_targets)
        ...
        if pos_mask.sum() > 0:
            ...
            pos_bbox_preds = bbox_preds[pos_mask] # All points with valid IDs
    ...
    def _loss(self, bbox_preds, cls_preds, points,
        ...
        for i in range(len(img_metas)):
            ...
            # print('*'*40, len(gt_bboxes), len(gt_labels), len(gt_bboxes[i]), len(gt_labels[i]))
...
@HEADS.register_module()
class TR3DHeadAdver(BaseModule):
    def __init__(self,
                 n_classes_ono,
                 n_classes_disc,
                 in_channels,
                 ...
                 bbox_loss=dict(type='AxisAlignedIoULoss', reduction='none'),
                 cls_loss=dict(type='FocalLoss', reduction='none'),
                 disc_loss=dict(type='FocalLoss', reduction='none'),
                 train_cfg=None,
                 test_cfg=None):
        super(TR3DHeadAdver, self).__init__()
        self.voxel_size = voxel_size
        self.assigner = build_assigner(assigner)
        self.bbox_loss = build_loss(bbox_loss)
        self.cls_loss = build_loss(cls_loss)
        self.disc_loss = build_loss(disc_loss)
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self._init_layers(n_classes_ono, n_classes_disc, in_channels, n_reg_outs)
        self.train_disc = True
        self.SMALL_OBJECTS = [3, 5, 17, 25, 29, 31, 32, 33, 34, 35]
        self.LARGE_OBJECTS = [4, 6, 7, 10, 12, 14, 15, 24, 36]
        self.UNSEEN_OBJECTS = [4, 10, 17, 29]

    def _init_layers(self, n_classes_ono, n_classes_disc, in_channels, n_reg_outs):
        self.bbox_conv = ME.MinkowskiConvolution(
            in_channels, n_reg_outs, kernel_size=1, bias=True, dimension=3)
        self.cls_conv = ME.MinkowskiConvolution(
            in_channels, n_classes_ono, kernel_size=1, bias=True, dimension=3)
        self.disc_conv = ME.MinkowskiConvolution(
            in_channels, n_classes_disc, kernel_size=1, bias=True, dimension=3)

    def init_weights(self):
        nn.init.normal_(self.bbox_conv.kernel, std=.01)
        nn.init.normal_(self.cls_conv.kernel, std=.01)
        nn.init.constant_(self.cls_conv.bias, bias_init_with_prob(.01))
        nn.init.normal_(self.disc_conv.kernel, std=.01)
        nn.init.constant_(self.disc_conv.bias, bias_init_with_prob(.01))
    ...
    def _forward_single(self, x):
        ...
        bbox_pred = torch.cat((reg_final[:, :3], reg_distance, reg_angle), dim=1)
        cls_pred = self.cls_conv(x).features
        disc_pred = self.disc_conv(x).features

        bbox_preds, cls_preds, disc_preds, points = [], [], [], []
        for permutation in x.decomposition_permutations:
            bbox_preds.append(bbox_pred[permutation])
            cls_preds.append(cls_pred[permutation])
            disc_preds.append(disc_pred[permutation])
            points.append(x.coordinates[permutation][:, 1:] * self.voxel_size)

        return bbox_preds, cls_preds, disc_preds, points

    def forward(self, x):
        bbox_preds, cls_preds, disc_preds, points = [], [], [], []
        for i in range(len(x)):
            bbox_pred, cls_pred, disc_pred, point = self._forward_single(x[i])
            bbox_preds.append(bbox_pred)
            cls_preds.append(cls_pred)
    ...
    def _loss_single(self,
        ...
        # cls loss
        n_classes_ono = cls_preds.shape[1]
        n_classes_disc = disc_preds.shape[1]
        ...
        if len(gt_labels) > 0:
            # cls_targets = torch.where(pos_mask, gt_labels[assigned_ids], n_classes)
            disc_targets = torch.where(pos_mask, gt_labels[assigned_ids], 1000)
        else:
            # cls_targets = gt_labels.new_full((len(pos_mask),), n_classes)
            disc_targets = gt_labels.new_full((len(pos_mask),), 1000)
        
        #Find unseen mask by using the generated cls_targets
        unseen_mask = torch.isin(disc_targets, torch.tensor(self.UNSEEN_OBJECTS).to(disc_targets.device))
        # Update assigned_ids to -1 for unseen classes
        assigned_ids[unseen_mask] = -1 # All unseen object ids should change to -1
        ...
        cls_targets = torch.where(torch.isin(disc_targets, torch.tensor(self.LARGE_OBJECTS).to(disc_targets.device)), 1, disc_targets)
        cls_targets = torch.where(torch.isin(disc_targets, torch.tensor(self.SMALL_OBJECTS).to(disc_targets.device)), 0, cls_targets)
        cls_targets = torch.where(torch.isin(disc_targets, torch.tensor([1000]).to(disc_targets.device)), n_classes_ono, cls_targets)
        
        disc_targets = torch.where(torch.isin(disc_targets, torch.tensor([1000]).to(disc_targets.device)), n_classes_disc, disc_targets)

        cls_loss = self.cls_loss(cls_preds, cls_targets)
        disc_loss = self.disc_loss(disc_preds, disc_targets)
        ...
        if pos_mask.sum() > 0:
            ...
            bbox_loss = None
        return bbox_loss, cls_loss, disc_loss, pos_mask

    def _loss(self, bbox_preds, cls_preds, disc_preds, points,
              gt_bboxes, gt_labels, img_metas):
        bbox_losses, cls_losses, disc_losses, pos_masks = [], [], [], []
        for i in range(len(img_metas)):
            bbox_loss, cls_loss, disc_loss, pos_mask = self._loss_single(
                bbox_preds=[x[i] for x in bbox_preds],
                cls_preds=[x[i] for x in cls_preds],
                disc_preds=[x[i] for x in disc_preds],
                points=[x[i] for x in points],
                img_meta=img_metas[i],
                gt_bboxes=gt_bboxes[i],
                gt_labels=gt_labels[i])
        ...
        ret_bbox_loss = torch.mean(torch.cat(bbox_losses))
        ret_cls_loss = torch.sum(torch.cat(cls_losses)) / torch.sum(torch.cat(pos_masks))
        ret_disc_loss = torch.sum(torch.cat(disc_losses)) / torch.sum(torch.cat(pos_masks))
        
        if self.train_disc:
            ret_bbox_loss.fill_(0)
            ret_cls_loss.fill_(0)
        else:
            ret_disc_loss = -ret_disc_loss
        
        return dict(
            bbox_loss=ret_bbox_loss,
            cls_loss=ret_cls_loss,
            disc_loss=ret_disc_loss,
            )

    def forward_train(self, x, gt_bboxes, gt_labels, img_metas):
        bbox_preds, cls_preds, disc_preds, points = self(x)
        return self._loss(bbox_preds, cls_preds, disc_preds, points,
                          gt_bboxes, gt_labels, img_metas)
